...
func (x *Beam) SolvePuzzle(inState []int, hMod fn, setState ssn, stateOptions gsn) {
	//make slice and limit nodes to k
	var nodeSlice []Node
	var evaluatedMoves [][]int
	nodeSlice = make([]Node, 1, x.K)

	//count the amount of nodes generated
	nodeCounter := 0

	//add the head node to evaluate
	nodeSlice[0] = Node{inState, hMod(inState), []string{"origin"}}
